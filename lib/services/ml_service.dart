...
class MLService {
  ...
  Future<List> setCurrentPrediction(XFile cameraImage, Face face) async {
    ...
    final _predictedData = List.from(output);

    return _predictedData;
  }

  Future<bool> predict(List modelData, List predictedData) async {
    return _searchResult(modelData, predictedData);
  }
  ...
  Future<imglib.Image> _cropFace(XFile image, Face faceDetected) async {
  ...
  Future<bool> _searchResult(List modelData, List predictedData) async {
    double minDist = 999;
    double currDist = 0.0;
